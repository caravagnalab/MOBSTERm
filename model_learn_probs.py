...
class mobster_MV():
    ...
    def set_prior_parameters(self):
        ...
        self.min_vaf = torch.tensor(0.08)
    ...
    def guide(self):
        ...
        # probs_pareto_param = pyro.sample("probs_pareto_param", BoundedPareto(self.pareto_L, alpha, self.pareto_H).sample())
        # probs_pareto_param = pyro.sample("probs_pareto_param", lambda: self.kmeans_centers, constraint=constraints.interval(self.pareto_L, self.pareto_H))
        probs_pareto_param = pyro.param("probs_pareto_param", lambda: torch.ones((K,D))*0.1, constraint=constraints.interval(self.pareto_L, self.pareto_H))
    ...
    def run_inference(self, num_iter = 2000, lr = 0.001):
        ...
        for name, value in pyro.get_param_store().named_parameters():
            if name in ["alpha_pareto_param", "delta_param", "probs_pareto_param"]:
                value.register_hook(
