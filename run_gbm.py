...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run with parameters --sample_name")
    parser.add_argument("--sample_name", type=str, required=True)

    args = parser.parse_args()
    sample_name = args.sample_name  # number of mutations
    
    data_folder = f'./paper_new/GBM_new/{sample_name}'
    data = pd.read_csv(f"./data/GBM_data/{sample_name}.csv")
    ...
    purity = [data['purity_primary'][0], data['purity_relapse'][0]]
