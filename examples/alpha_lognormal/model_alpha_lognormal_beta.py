...
class mobster_MV():
    ...
    def log_beta_par_mix(self, probs_pareto, delta, probs_beta):
        # relaxed one hot
        # delta -> D x 2
        ...
        delta_beta = torch.log(delta[:, 1]) + dist.Binomial(total_count=self.DP, probs = probs_beta).log_prob(self.NV)
    ...
    def m_total_lk(self, probs_pareto, alpha, probs_beta,  weights, delta):
        ...
        for k in range(self.K):
            lk[k, :] = torch.log(weights[k]) + self.log_beta_par_mix(probs_pareto[k, :], delta[k, :, :], probs_beta[k, :]).sum(axis=1) # sums over the data dimensions (columns)
    ...
    def model(self):
        ...
        with pyro.plate("plate_dims", D):
            with pyro.plate("plate_probs", K):
                ...
                a_beta = phi_beta * k_beta
                b_beta = (1-phi_beta) * k_beta
                probs_beta = pyro.sample("probs_beta", dist.Beta(a_beta, b_beta))
        ...
        with pyro.plate("plate_data", len(NV)):
            ...
            pyro.factor("lik", self.log_sum_exp(self.m_total_lk(probs_pareto, alpha, probs_beta, weights, delta)).sum())
    ...
    def guide(self):
        ...
        a_beta = phi_beta_param * k_beta_param
        b_beta = (1-phi_beta_param) * k_beta_param
        ...
        with pyro.plate("plate_dims", D):
            with pyro.plate("plate_probs", K):
                ...
                pyro.sample("probs_beta", dist.Beta(a_beta, b_beta))
    ...
    def fit(self, num_iter = 2000, lr = 0.001):
        ...
        for i in range(num_iter):
            ...
            probs_beta = dist.Beta(a_beta, b_beta).sample()
            lks = self.log_sum_exp(self.m_total_lk(probs_pareto,
                                                   params["alpha_pareto_param"], probs_beta, 
    ...
    def beta_lk2(self, a_beta, b_beta):
        # Doesn't work if K = 1
        LINSPACE = 2000
        # print(alpha)
        # print(alpha.shape)
    ...
    def log_beta_par_mix2(self, delta, alpha, a_beta, b_beta):
        ...
        delta_beta = torch.log(delta[:, 1]) + self.beta_lk2(a_beta, b_beta) # 1x2 tensor
