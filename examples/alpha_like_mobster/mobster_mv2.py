...
class mobster_MV():
    ...
    def set_prior_parameters(self):
        ...
        self.alpha_precision_concentration = 2
        self.alpha_precision_rate=0.01
    ...
    def guide(self):
        ...
        # ------------------------Alpha------------------------- #
        a_prior = pyro.param("tail_mean_param", lambda: torch.ones((K,D))*2,  constraint=constraints.positive)
        alpha_precision_par = pyro.param("alpha_prec_param", lambda: torch.ones((K,D))*2, constraint=constraints.positive)
        alpha_param = pyro.param("alpha_param", dist.LogNormal(torch.log(torch.tensor(2.)), 
                                                               torch.tensor(0.5)).sample((K,D)), constraint=constraints.positive)
        # ------------------------Alpha------------------------- #
        
        phi_beta_param = pyro.param("phi_beta_param", lambda: self.kmeans_centers, constraint=constraints.interval(0., self.max_vaf))
        k_beta_param = pyro.param("k_beta_param", lambda: torch.ones((K,D))*50, constraint=constraints.positive)
