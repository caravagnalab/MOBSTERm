...
class mobster_MV():
    ...
    def set_prior_parameters(self):
        ...
        self.alpha = torch.ones(self.K,self.NV.shape[1])*2
    ...
    def model(self):
        ...
        with pyro.plate("plate_dims", D):
            with pyro.plate("plate_probs", K):
                ...
                probs_pareto = pyro.sample("probs_pareto", BoundedPareto(pareto_L, self.alpha, pareto_H)) # probs_pareto is a K x D tensor

        # Data generation
        with pyro.plate("plate_data", len(NV)):
            # .sum() sums over the data because we have a log-likelihood
            pyro.factor("lik", self.log_sum_exp(self.m_total_lk(probs_beta, probs_pareto, self.alpha, a_beta, b_beta, weights, delta)).sum())
    ...
    def get_parameters(self):
        ...
        params["alpha_pareto"] = self.alpha
    ...
    def fit(self, num_iter = 2000, lr = 0.001):
        ...
        for name, value in pyro.get_param_store().named_parameters():
            if name in ["probs_beta_param", "probs_pareto_param", "delta_param"]:
                value.register_hook(
        ...
        for i in range(num_iter):
            ...
            lks = self.log_sum_exp(self.m_total_lk(probs_beta, probs_pareto, self.alpha, a_beta, b_beta, weights, delta)).sum()
            self.lks.append(lks)
