...
class mobster_MV():
    ...
    def model(self):
        ...
        with pyro.plate("plate_dims", D):
            with pyro.plate("plate_probs", K):
                ...
                alpha = pyro.sample("alpha_pareto", dist.LogNormal(torch.log(alpha_prior), self.alpha_pareto_std))
    ...
    def guide(self):
        ...
        with pyro.plate("plate_dims", D):
            with pyro.plate("plate_probs", K):
                ...
                alpha = pyro.sample("alpha_pareto", dist.LogNormal(torch.log(alpha_prior), self.alpha_pareto_std)) # here because we need to have K x D samples
    ...
    def fit(self, num_iter = 2000, lr = 0.001):
        ...
        for name, value in pyro.get_param_store().named_parameters():
            if name in ["alpha_prior_param", "delta_param"]:
                value.register_hook(
...
# Model selection
def BIC(self, lk, params):
    n_params = calculate_number_of_params(params)
    n = self.NV.shape[0]
    lk = self.compute_final_lk()
