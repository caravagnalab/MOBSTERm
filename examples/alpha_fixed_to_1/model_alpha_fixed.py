...
class mobster_MV():
    ...
    def initialize_delta(self, phi_beta, k_beta, alpha):
        ...
        print(alpha.shape)
        print(self.pareto_L.shape)
        pareto_lk = BoundedPareto(self.pareto_L, alpha, torch.ones(self.K, self.NV.shape[1])).log_prob(self.kmeans_centers_no_noise) #+ dist.Binomial(total_count=self.DP, probs = self.kmeans_centers).log_prob(self.NV)
        print("Beta: ", beta_lk)
    ...
    def set_prior_parameters(self):
        ...
        # alpha_pareto (normal)
        self.alpha_pareto_mean = torch.ones((self.K, self.NV.shape[1]))*1.
    ...
    def model(self):
        ...
        with pyro.plate("plate_dims", D):
            with pyro.plate("plate_probs", K):
                ...
                probs_pareto = pyro.sample("probs_pareto", BoundedPareto(self.pareto_L, self.alpha_pareto_mean, self.pareto_H)) # probs_pareto is a K x D tensor

        # Data generation
        with pyro.plate("plate_data", len(NV)):
            # .sum() sums over the data because we have a log-likelihood
            pyro.factor("lik", self.log_sum_exp(self.m_total_lk(probs_pareto, self.alpha_pareto_mean, a_beta, b_beta, weights, delta)).sum())
    ...
    def guide(self):
        ...
        with pyro.plate("plate_dims", D):
            with pyro.plate("plate_probs", K):
                ...
                pyro.sample("probs_pareto", BoundedPareto(self.pareto_L, self.alpha_pareto_mean, self.pareto_H))
    ...
    def get_parameters(self):
        ...
        params["alpha_pareto_param"] = self.alpha_pareto_mean
        ...
        for k in range(self.K):
            for d in range(self.NV.shape[1]):
                ...
                if maxx == 1: # beta
                    ...
                    probs[k,d] = BoundedPareto(self.pareto_L[k,d], alpha[k,d], self.pareto_H).sample()
        return probs
    ...
    def pareto_lk2(self, alpha):
        ...
        # print(alpha.shape)
        # x = torch.linspace(0.01, 0.999, LINSPACE)
        scale = torch.min(self.pareto_L)
        x = torch.linspace(scale, self.max_vaf, LINSPACE) # sampled "probability" values (possibili valori discreti sul dominio di integrazione)
        y_1 = torch.stack([
            BoundedPareto(scale, alpha[0], self.pareto_H).log_prob(x).exp(),  # dim 0
            BoundedPareto(scale, alpha[1], self.pareto_H).log_prob(x).exp()   # dim 1
        ], dim=1)
        y_2 = torch.stack([
            dist.Binomial(probs=x.repeat([self.NV.shape[0], 1]).reshape([LINSPACE, -1]), total_count=self.DP[:, 0]).log_prob(self.NV[:, 0]).exp(),
            dist.Binomial(probs=x.repeat([self.NV.shape[0], 1]).reshape([LINSPACE, -1]), total_count=self.DP[:, 1]).log_prob(self.NV[:, 1]).exp()
