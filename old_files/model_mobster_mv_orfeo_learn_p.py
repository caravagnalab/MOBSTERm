...
class mobster_MV():
    ...
    def compute_kmeans_centers(self):
        ...
        best_centers[best_centers <= torch.min(self.phi_beta_L)] = torch.min(self.phi_beta_L)
    ...
    def model(self):
        ...
        with pyro.plate("plate_data", len(NV)):
            ...
            pyro.factor("lik", self.log_sum_exp(self.m_total_lk(probs_pareto, self.alpha_factor*alpha, a_beta, b_beta, weights, w)).sum())
    ...
    def guide(self):
        ...
        probs_pareto_param = pyro.param("probs_pareto_param", lambda: self.kmeans_centers, constraint=constraints.interval(self.min_vaf, self.phi_beta_H))
    ...
    def compute_entropy(self, params):
        posts = params["responsib_sampling_p"] # K x N
        entropy = 0
    ...
    def run_inference(self, num_iter = 2000, lr = 0.001):
        ...
        for name, value in pyro.get_param_store().named_parameters():
            if name in ["alpha_pareto_param", "delta_param", "weights_param", "probs_pareto_param"]:
                value.register_hook(
        ...
        print("Learned p: ", self.params["probs_pareto_param"])
        final_lk = self.compute_posteriors()
        print("Inference lk: ", self.lks[-1])
        print("Lk with learned params: ", self.log_sum_exp(final_lk).sum())
    ...
    def compute_final_lk(self):
        ...
        """
        probs_pareto = self.params["probs_pareto_param"]
    ...
    def compute_posteriors(self):
        ...
        self.params["responsib_sampling_p"] = res # qui non dovrebbe cambiare niente per le private perchè i punti sono già sommati sulle dimensioni
