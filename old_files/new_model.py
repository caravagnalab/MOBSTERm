...
def fit(NV = None, DP = None, num_iter = 2000, K = [], purity=1, 
        seed=[123,1234], par_threshold = 0.005, loss_threshold = 0.01, 
        lr = 0.01, savefig = False, data_folder = None, choice_init_centers = 'kmeans'):
    """
        ...
        if curr_k != 0:
            ...
            for curr_seed in seed:
                ...
                curr_mb.append(mobster_MV(NV, DP, K = curr_k, purity = purity, 
                                            ...
                                            data_folder = data_folder, choice_init_centers = choice_init_centers))
...
class mobster_MV():
    def __init__(self, NV = None, DP = None, K = 1, purity=1, seed=[123,1234], par_threshold = 0.005, 
                 loss_threshold = 0.01, savefig = False, data_folder = None, choice_init_centers = 'kmeans'):
        """
        Parameters:
        ...
        self.choice_init_centers = choice_init_centers
    ...
    def compute_kmeans_centers(self):
        ...
        if self.choice_init_centers == 'kmeans':    
            # Gaussian noise
            mean = 0
            std_dev = 0.08
            D = self.NV.shape[1]
            ...
            self.kmeans_centers_delta = best_centers.clone()
            self.kmeans_centers_delta[self.kmeans_centers_delta <= 0] = 1e-10 # it could be 0 because now we are considering the private mutations 
            self.kmeans_centers_delta[self.kmeans_centers_delta >= self.max_vaf] = self.max_vaf 
        
            
            # Clip probabilities in suitable range [phi_beta_L, phi_beta_H]
            ...
            plt.show()
            plt.close()
        elif self.choice_init_centers == 'random':
            random_centers = dist.Uniform(self.phi_beta_L, self.phi_beta_H).sample([self.K, self.NV.shape[1]])
            self.kmeans_centers = random_centers
        # elif self.choice_init_centers == 'minimize_loss':
        
        

    def initialize_delta(self):
        centers = self.kmeans_centers_no_noise
        a_beta = self.get_a_beta(centers, self.k_beta_init)
        b_beta = self.get_b_beta(centers, self.k_beta_init)
        beta_lk = dist.Beta(a_beta, b_beta).log_prob(centers)
        ...
        pareto_lk = BoundedPareto(self.pareto_L, self.alpha_pareto_mean, self.pareto_H).log_prob(centers)

        zeros_lk = dist.Beta(self.a_beta_zeros, self.b_beta_zeros).log_prob(centers)
    ...
    def set_prior_parameters(self):
        ...
        self.alpha_pareto_mean = torch.tensor(1.4)
        self.alpha_pareto_std = torch.tensor(0.8)
    ...
    def guide(self):
        ...
        # phi_beta_param = pyro.param("phi_beta_param", lambda: dist.Uniform(self.phi_beta_L, self.phi_beta_H).sample([K,D]), constraint=constraints.interval(self.phi_beta_L, self.phi_beta_H))
    ...
    def init_loc_fn(self, site):
        ...
        K, D = self.K, self.NV.shape[1]
        if site_name == "weights":
            param = dist.Dirichlet(torch.ones(K)).sample()
        if site_name == "delta":
            param = dist.Dirichlet(torch.ones(3)).sample([K, D]).reshape(K, D, 3)
        if site_name == "phi_beta":
            param = dist.Uniform(self.phi_beta_L, self.phi_beta_H).sample([K,D])
        if site_name == "k_beta":
            param = torch.ones((K,D))*self.k_beta_init
        if site_name == "alpha_pareto":
            param = torch.ones((K,D))*self.alpha_pareto_init
    ...
    def min_loss_init(self, seed, lr, final = False):
        # qui devi fare una inizializzazione random di self.kmeans_centers e di conseguenza di delta e weights
        pyro.set_rng_seed(seed)
        pyro.clear_param_store()
        global_guide = AutoDelta(poutine.block(self.model,
                                       expose=["weights","phi_beta","delta", "k_beta", 
                                               "alpha_pareto", "probs_pareto"]),
                        init_loc_fn=self.init_loc_fn)
        svi = pyro.infer.SVI(self.model, global_guide, pyro.optim.Adam({"lr": lr}), pyro.infer.TraceGraph_ELBO())
        if final == False:
            return svi.loss(self.model, global_guide)
        else:
            return svi
    ...
    def run_inference(self, num_iter = 2000, lr = 0.001):
        if self.choice_init_centers == 'kmeans' or self.choice_init_centers == 'random':
            pyro.clear_param_store()
            pyro.set_rng_seed(self.seed)
            torch.manual_seed(self.seed)
            np.random.seed(self.seed)
            ...
            pyro.clear_param_store()
            print('here')
            loss, seed = min((self.min_loss_init(s, lr), s) for s in range(100))
            print('here')
            svi = self.min_loss_init(seed, lr, True)
            svi.step()
            # self.min_loss_init()
    ...
    def compute_euclidean_distance(self, t1, t2):
        ...
        t1_flat = t1.flatten()
        t2_flat = t2.flatten()
        distance = torch.norm(t1_flat - t2_flat)
        return (distance/torch.norm(t1_flat)).item()
