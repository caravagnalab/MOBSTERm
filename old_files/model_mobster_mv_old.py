...
class mobster_MV():
    ...
    def compute_kmeans_centers(self):
        ...
        best_centers[best_centers < self.phi_beta_L] = torch.min(self.phi_beta_L)
    ...
    def pareto_lk(self, d, alpha):
        ...
        # idx = [200, 201, 202, 203]
        # print("Paretobin integr", paretobin2[idx])
        ...
        # pareto = simpson((y_1.reshape([LINSPACE, 1]) * y_2).detach().numpy(), x=x.numpy(), axis=0)
        # paretobin3 = torch.tensor(pareto).log()
        # print("Paretobin sampled",paretobin[idx])
    ...
    def m_total_lk(self, probs_pareto, alpha, a_beta, b_beta, weights, delta, which = 'inference'):
        ...
        for k in range(self.K):
            if which == 'inference':
                ...
                print("K = ", k)
                lk[k, :] = torch.log(weights[k]) + self.log_beta_par_mix_posteriors(delta[k, :, :], alpha[k, :], a_beta[k, :], b_beta[k, :]).sum(axis=1) # sums over the data dimensions (columns)
                # idx = [200, 201, 202, 203]
                # print(lk[k, idx])                                                                                                        # put on each column of lk a different data; rows are the clusters
        return lk
